...
# Train with Detic as a 2D recall enhancer:
#   - GroundingSAM/ng-weighted boxes supervise full 3D.
#   - Detic-only boxes supervise 2D classification/box heads.
#   - Detic 3D factor weights can be exactly zero, so noisy dims/yaw do not
#     leak into CubeHead. Depth consistency follows pseudo_weight_z.

PL_FOLDER="${PL_FOLDER:-Omni3D_pl-detic-2d-recall}"
OUTPUT_DIR="${OUTPUT_DIR:-output/training/SUN_detic_2d_recall_depthreg}"
...
CUDA_VISIBLE_DEVICES="${CUDA_VISIBLE_DEVICES:-0,1}" python tools/train_net.py \
  ...
  MODEL.ROI_CUBE_HEAD.FACTORIZED_PSEUDO_WEIGHT_MIN 0.0 \
  ...
  MODEL.ROI_CUBE_HEAD.DEPTH_CONSISTENCY_USE_FACTORIZED_Z_WEIGHT True \
