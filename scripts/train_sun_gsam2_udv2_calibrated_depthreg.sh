...
PL_FOLDER="${PL_FOLDER:-Omni3D_pl-gsam2-udv2-calibrated}"
OUTPUT_DIR="${OUTPUT_DIR:-output/training/SUN_gsam2_udv2_calibrated_depthreg}"
DEPTH_ROOT="${DEPTH_ROOT:-pseudo_label_gsam2_udv2_calibrated/SUNRGBD}"
DIST_URL="${DIST_URL:-tcp://0.0.0.0:12395}"
NUM_GPUS="${NUM_GPUS:-2}"

# Stable training route for the calibrated full-replacement pseudo labels:
# - cached UniDepthV2 depth supervises front-surface consistency
# - factorized pseudo weights keep center/depth stronger than dims/yaw
# - no RSH / CoP-GS / ZEM / latentbox, so the comparison isolates pseudo-label quality
