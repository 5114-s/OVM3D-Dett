...
# Pseudo labels from the upgraded 2D/depth front-end:
#   GroundingSAM2 cache + UniDepthV2 depth
#   -> DFU/PCA 3D anchor
#   -> depth/projection/PAG quality score
#   -> factorized pseudo weights
#
# This route intentionally does not use Detic and does not copy old 3D boxes
# with --use_source_geometry_anchor. It tests whether the upgraded front-end
# improves the pseudo labels by itself.

PSEUDO_ROOT="${PSEUDO_ROOT:-pseudo_label_gsam2_udv2}"
OUT_DIR="${OUT_DIR:-datasets/Omni3D_pl-gsam2-udv2-ng}"
STATS_DIR="${STATS_DIR:-outputs/gsam2_udv2_ng_stats}"
...
echo "Done. Pseudo labels: ${OUT_DIR}"
