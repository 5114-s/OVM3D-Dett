...
# Build an oracle-2D Boxer source for Route A.
# It uses GT 2D boxes from Omni3D JSON as 2D proposals, but does not use GT 3D
# values as source boxes.  True 3D boxes are only consumed later as targets for
# base-category LiftHead training.

OUT_ROOT="${OUT_ROOT:-outputs/boxer_gt2d_source_sun}"
...
CUDA_VISIBLE_DEVICES="${CUDA_VISIBLE_DEVICES:-0}" python tools/run_boxer_omni3d.py \
  ...
  --output_json "${OUT_ROOT}/SUNRGBD_train_boxer_gt2d.json" \
  --box_source json \
...
CUDA_VISIBLE_DEVICES="${CUDA_VISIBLE_DEVICES:-0}" python tools/run_boxer_omni3d.py \
  ...
  --output_json "${OUT_ROOT}/SUNRGBD_val_boxer_gt2d.json" \
  --box_source json \
  --depth_source original_unidepth \
  --dataset "${DATASET}" \
  --split val \
