...
OUTPUT_DIR="${OUTPUT_DIR:-output/training/SUN_ng_weighted_depthreg_corneraux}"
DEPTH_ROOT="${DEPTH_ROOT:-pseudo_label/SUNRGBD}"
DIST_URL="${DIST_URL:-tcp://0.0.0.0:12378}"
NUM_GPUS="${NUM_GPUS:-2}"

# Stable OVM3D-Det route plus a MoCA3D-inspired projected corner-depth
# auxiliary loss. It keeps the original CubeHead prediction path unchanged and
# only adds image-plane corner/depth consistency during training.
CUDA_VISIBLE_DEVICES="${CUDA_VISIBLE_DEVICES:-0,1}" python tools/train_net.py \
  ...
  MODEL.ROI_CUBE_HEAD.LOSS_W_PROJECTED_CORNER_2D 0.03 \
  MODEL.ROI_CUBE_HEAD.LOSS_W_PROJECTED_CORNER_DEPTH 0.03 \
  MODEL.ROI_CUBE_HEAD.PROJECTED_CORNER_MAX_LOSS 1.0 \
  MODEL.ROI_CUBE_HEAD.USE_CORNER_HEATMAP_AUX True \
  MODEL.ROI_CUBE_HEAD.LOSS_W_CORNER_HEATMAP 0.02 \
