...
PL_FOLDER="${PL_FOLDER:-Omni3D_pl-sor-refine}"
OUTPUT_DIR="${OUTPUT_DIR:-output/training/SUN_sor_refine_depthreg}"
DEPTH_ROOT="${DEPTH_ROOT:-pseudo_label/SUNRGBD}"
DIST_URL="${DIST_URL:-tcp://0.0.0.0:12366}"
NUM_GPUS="${NUM_GPUS:-2}"

# Stable evaluation route for SOR-refined pseudo labels:
# - no RSH and no CoP/GS, so the ablation mainly measures pseudo-label quality
# - cached UniDepth is used only as a training-time depth consistency regularizer
