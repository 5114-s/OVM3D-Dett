...
# Fast Detic strict + geometry-quality route.
#
# Compared with build_detic_dfu_imov3d_moca_sun.sh, this keeps the high-value
# quality signals but disables the slowest per-instance point-cloud operations:
#   - no frustum DBSCAN
#   - no radius outlier filter
#   - no projected-corner sample list storage
#
# Use this first to test whether the Detic pseudo-label quality improves before
# spending many hours on the full DBSCAN/radius version.

OUT_DIR="${OUT_DIR:-datasets/Omni3D_pl-detic-dfu-imov3d-moca-fast}"
STATS_DIR="${STATS_DIR:-outputs/detic_dfu_imov3d_moca_fast_stats}"
...
COMMON_ARGS=(
  ...
  # Fast DFU3D-style cleanup: depth edges + depth-coherent mask selection +
  # robust depth/axis filtering inside build_dfu_robust_pca_pseudolabels.py.
  ...
  # ImOV3D-style geometry/priors, kept because this is computed per image or
  # as cheap scalar quality weights.
  ...
  # MoCA3D-style PAG score, without storing large per-corner debug payloads.
